...
#import "AthUserDbManager.h"
#import "AthDbTool.h"

@implementation AthUserDbManager{
...
+(instancetype)shareManager{
    ...
    static AthUserDbManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [AthUserDbManager new];
    });
...
-(void)initSafeDB:(NSString*)uid{
    ...
    self.uid = uid;
    NSString *dbPath = [[self getDbpath] stringByAppendingPathComponent:@"khl_user.db"];
    ATLog(@"%@",dbPath);
...
-(void)creatTable{
    ...
    NSString *tablePath = [[NSBundle mainBundle] pathForResource:@"AthDb.plist" ofType:nil];
    ...
    [_queueDb inDatabase:^(FMDatabase * _Nonnull db) {
        ...
        for (NSString *table in _tables) {
            ...
            //
            if ([table isEqualToString:@"Downloding"]) {
                
                NSDictionary *feilDic = [self getTableInfo:table db:db];
                NSDictionary *souDic = _tables[table];
                
                for (NSString *newFeil in souDic) {
                    
                    if (!feilDic[newFeil]) {
                        
                        BOOL resSet =  [db executeUpdate:AthStrAppend(@"ALTER TABLE %@ ADD COLUMN %@ %@",table,newFeil,souDic[newFeil])];
                        ATLog(@"%d",resSet);
...
-(void)searchData:(NSDictionary*)model resault:(void(^)(NSArray* datas))res table:(NSString*)tablename orderFile:(NSString*)file adsc:(NSString*)adsc{
    ...
    [_queueDb inDatabase:^(FMDatabase * _Nonnull db) {
        ...
        sql = [sql stringByAppendingString:AthStrAppend(@"order by %@ %@ ",file,adsc)];
...
-(void)searchData:(NSDictionary*)model resault:(void(^)(NSArray* datas))res table:(NSString*)tablename orderFile:(NSString*)file adsc:(NSString*)adsc limit:(NSInteger)limit{
    ...
    [_queueDb inDatabase:^(FMDatabase * _Nonnull db) {
        ...
        if (limit > 0) {
            ...
            sql = [sql stringByAppendingString:AthStrAppend(@"limit %ld",limit)];
        }
...
-(NSString*)getDbpath{
    ...
    NSString *dbStr = AthStrAppend(@"Db/%@",self.uid);
